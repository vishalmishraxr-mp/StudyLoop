...
exports.createCourse = async (req, res) => {
    try {
        ...
        if (!courseName || !courseDescription || price === undefined || price === null || !tag || !category || !thumbnail) {
...
exports.showAllCourses = async (req, res) => {
    try {
        const allCourses = await Course.find({ status: "Published" })
            .select("courseName courseDescription price thumbnail instructor ratingAndReview studentsEnrolled category tag status")
        ...
        console.error("showAllCourses:", error);
        return res.status(500).json({
            success: false,
...
exports.getAllDetails = async (req, res) => {
    try {
        ...
        console.error("getAllDetails:", error);
        return res.status(500).json({
            success: false,
...
exports.changeCourseStatus = async (req, res) => {
    try {
        ...
        if (!courseId || !["Draft", "Published"].includes(status)) {
            return res.status(400).json({
                success: false,
                message: "A valid courseId and status (Draft or Published) are required",
            });
        }
        ...
        console.error("changeCourseStatus:", error);
        return res.status(500).json({
            success: false,
