...
const Navbar = () => {
    ...
    return (
        <div className="relative z-50 flex h-14 items-center justify-center border-b border-b-slate-700 bg-stone-950">
            ...
            <div className="flex w-11/12 max-w-maxContetnt items-center justify-between">
                            ...
                            <li key={index}>
                                ...
                                {link.title === "Catalog" ? (
                                    <div className="group relative flex items-center gap-1 cursor-pointer">
                                        ...
                                        {/* DESKTOP DROPDOWN */}
                                        <div className="invisible absolute left-1/2 top-full z-50 mt-2 w-[300px] -translate-x-1/2 rounded-xl bg-white p-4 text-black opacity-0 shadow-xl transition-all duration-300 ease-out group-hover:visible group-hover:opacity-100">
                ...
                <div className="flex items-center gap-x-3 sm:gap-x-4">
                    ...
                    <button
                        ...
                        className="rounded-lg p-2 text-gray-300 transition-all duration-200 hover:bg-gray-800 hover:text-white md:hidden"
            ...
            <div
                className={`absolute top-14 left-0 w-full origin-top overflow-hidden border-b border-gray-800 bg-stone-950 shadow-2xl transition-all duration-500 ease-out md:hidden ${
                    mobileMenuOpen
                        ? "visible max-h-[600px] translate-y-0 scale-y-100 opacity-100"
                        : "invisible pointer-events-none max-h-0 -translate-y-2 scale-y-95 opacity-0"
                }`}
            >
                ...
                <div className="p-5">
                            ...
                            <li key={index}>
                                ...
                                {link.title === "Catalog" ? (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-1 gap-1 border-l-2 border-yellow-400 pl-2">
                                            ...
                                            {subLinks.length > 0 ? (
                                                subLinks.map((subLink) => (
                                                    <Link
                                                        key={subLink._id}
                                                        to={`/catalog/${subLink._id}`}
                                                        className="py-1.5 text-sm text-gray-200 transition-colors duration-200 hover:text-yellow-400"
                                                    >
                                                        {subLink.name}
                                                    </Link>
                                                ))
                                            ) : (
                                                <p className="py-1.5 text-sm text-gray-500">
                                                    Loading categories...
                                                </p>
                                            )}
                    ...
                    {/* MOBILE LOGIN / SIGNUP */}
                    {!token && (
                        <div className="mt-6 flex flex-col gap-3 border-t border-gray-800 pt-4 sm:hidden">

                            <Link
                                to="/login"
                                className="w-full rounded-lg border border-gray-700 bg-gray-800 py-2.5 text-center text-sm font-semibold text-white transition-colors duration-200 hover:bg-gray-700"
                            >
                                Log in
                            </Link>
                            ...
                            <Link
                                to="/signup"
                                className="w-full rounded-lg bg-yellow-400 py-2.5 text-center text-sm font-semibold text-black transition-colors duration-200 hover:bg-yellow-500"
                            >
                                Sign up
                            </Link>
